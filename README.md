...
Сделать стиль, чтоб хорошо сочеталось
Более функцианальный калькулятор

# Как запустить проект:
1. Переходим в папку calculator (cd calculator)
2. Добавляем папку cmd (mkdir cmd)
3. cd cmd
4. Дальше в папке cmd пишем: go run main.go
5. Или просто в редакторе запускаем файл main.go и он работает
# Контакты для связи:
